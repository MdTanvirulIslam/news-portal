...
@section('content')
    <div class="row">
        <div class="col-12">
            <form action="{{ route('admin.rss-feeds.update', $rssFeed) }}" method="POST" id="edit-rss-form">
                @csrf
                @method('PUT')
                    ...
                    <div class="card-header">
                        <div class="d-flex justify-content-between align-items-center">
                            <h3 class="card-title mb-0">
                                <i class="fas fa-edit"></i> Edit RSS Feed
                            </h3>
                            <div class="text-muted small">
                                Last fetched: {{ $rssFeed->last_fetched_at ? $rssFeed->last_fetched_at->diffForHumans() : 'Never' }}
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-12 mb-3">
                                ...
                                <input type="url" name="feed_url" class="form-control @error('feed_url') is-invalid @enderror"
                                       value="{{ old('feed_url', $rssFeed->feed_url) }}" required>
                                @error('feed_url')
                                <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="number" name="import_limit" class="form-control @error('import_limit') is-invalid @enderror"
                                       value="{{ old('import_limit', $rssFeed->import_limit) }}" min="1" max="100" required>
                                @error('import_limit')
                                <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                                ...
                                <div class="card border">
                                    <div class="card-body p-3">
                                        <div class="d-flex justify-content-between">
                                            <span><i class="fas fa-download text-primary"></i> Total Imported:</span>
                                            <strong>{{ $rssFeed->total_imported }}</strong>
                                        </div>
                                        <div class="d-flex justify-content-between mt-2">
                                            <span><i class="fas fa-clock text-info"></i> Last Fetch:</span>
                                            <strong>{{ $rssFeed->last_fetched_at ? $rssFeed->last_fetched_at->format('M d, Y H:i') : 'Never' }}</strong>
                                        </div>
                                    </div>
                                </div>
                            </div>
                                    ...
                                    <div class="col-md-6">
                                        <div class="form-check form-switch">
                                            <input class="form-check-input" type="checkbox" name="is_active" id="is_active"
                                                   value="1" {{ old('is_active', $rssFeed->is_active) ? 'checked' : '' }}>
                                    ...
                                    <div class="col-md-6">
                                        <div class="form-check form-switch">
                                            <input class="form-check-input" type="checkbox" name="auto_import" id="auto_import"
                                                   value="1" {{ old('auto_import', $rssFeed->auto_import) ? 'checked' : '' }}>
                    ...
                    <div class="card-footer">
                        <div class="d-flex justify-content-between">
                            ...
                            <button type="submit" class="btn btn-primary" id="submit-btn">
                                <i class="fas fa-save"></i> Update RSS Feed
                            </button>
                        </div>
                    </div>
                </div>
...
@section('scripts')
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#edit-rss-form').on('submit', function() {
